...
### Template Provider
Source of block templates. Typically a Bitcoin Core full node. Provides `NewTemplate` messages with block data. In SRI, accessed via Bitcoin Core IPC or Sv2Tp protocol.
...
- **JDC** bridges between Bitcoin Core (template source) and Pool (template consumer)
