---
name: sv2pi
description: Agentic deployment of the Stratum V2 Reference Implementation (SRI) for Bitcoin mainnet. Use when deploying or managing Docker-based SRI apps (pool_sv2, jd_client_sv2, translator_sv2, sv2_tp) alongside Bitcoin Core with IPC. Also use for crash diagnostics, health monitoring of already-deployed SRI instances, serving/publishing the sv2pi operations vault via Quartz 4 over WireGuard, and checking PPQ credit balance for the LLM provider. Scope is strictly production mainnet — not for development, testing, or devnet work.
---
