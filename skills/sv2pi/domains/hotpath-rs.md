...
Check for a stale container with the old image (missing `--features hotpath`). Rebuild with `--no-build` to skip clone.

### Source Code for Log Comparison

The hotpath fork lives at `https://github.com/SV2-bot/sv2-apps`. The deploy script clones it to `/tmp/sv2-apps-hotpath-v$VERSION`. Use this clone's source for log comparison, not `stratum-mining/sv2-apps` — the fork may diverge in log output due to `--features hotpath`.
